...
class WC_Gateway_TradeSafe extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		$this->form_fields = array(
            ...
            'first_name' => array(
                'title'         => __('First Name', 'woocommerce-gateway-tradesafe'),
                'type'          => 'text',
                'placeholder'   => 'Example: John',
				'default'       => '',
            ),
            'last_name' => array(
                'title'         => __('Last Name', 'woocommerce-gateway-tradesafe'),
                'type'          => 'text',
                'placeholder'   => 'Example: Smith',
                'default'       => '',
            ),
            'email' => array(
                'title'         => __('Email', 'woocommerce-gateway-tradesafe'),
                'type'          => 'email',
                'placeholder'   => 'Example: user@example.com',
                'default'       => get_option( 'admin_email' ),
            ),
            'mobile_country' => array(
                'title'         => __('Country', 'woocommerce-gateway-tradesafe'),
                'type'          => 'text',
                'placeholder'   => 'Example: ZA',
                'default'       => 'ZA',
            ),
            'mobile' => array(
                'title'         => __('Mobile', 'woocommerce-gateway-tradesafe'),
                'type'          => 'number',
                'placeholder'   => 'Example: 0123456789',
                'default'       => '',
            ),
            'id_number' => array(
                'title'         => __('SA ID Number', 'woocommerce-gateway-tradesafe'),
                'type'          => 'text',
                'placeholder'   => 'Example: 1122334444555',
                'default'       => '',
            ),
			'send_debug_email' => array(
				'title'   => __( 'Send Debug Emails', 'woocommerce-gateway-tradesafe' ),
				'type'    => 'checkbox',
				'label'   => __( 'Send debug e-mails for transactions through the TradeSafe gateway (sends on successful transaction as well).', 'woocommerce-gateway-tradesafe' ),
				'default' => 'yes',
			),
			'debug_email' => array(
				'title'       => __( 'Who Receives Debug E-mails?', 'woocommerce-gateway-tradesafe' ),
				'type'        => 'text',
				'description' => __( 'The e-mail address to which debugging error e-mails are sent when in test mode.', 'woocommerce-gateway-tradesafe' ),
				'default'     => get_option( 'admin_email' ),
			),
	...
	public function admin_options() {
		...
		?>
			...
			<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'woocommerce-gateway-tradesafe' ); ?></strong> <?php /* translators: 1: a href link 2: closing href */ echo sprintf( __( 'Choose South African Rands as your store currency in %1$sGeneral Settings%2$s to enable the TradeSafe Gateway.', 'woocommerce-gateway-tradesafe' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=general' ) ) . '">', '</a>' ); ?></p></div>
			<?php
		}
	}
	...
	public function generate_tradesafe_form( $order_id ) {
		...
		if ($order->meta_exists('tradesafe_id')) {
            ...
            if ('agent' === $this->get_option('service_role')) {
                ...
                $data["seller"] = array(
                    "first_name" => "Seller",
                    "email" => "seller@tradesafe.co.za",
                    "mobile_country" => "ZA",
                    "mobile" => "0100000002",
                    "id_number" => "1901015000084",
                    "bank" => "ABC",
                    "number" => "0123456789",
                    "branch_code" => "123456",
                    "type" => "CHEQUE"
